...
extension DetailSectionModel: SectionModelType {
    typealias Item = QuizDetailSectionModel
    
    var items: [QuizDetailSectionModel] {
        switch self {
    ...
    init(original: DetailSectionModel, items: [QuizDetailSectionModel]) {
        switch original {
        case .detail(let item):
